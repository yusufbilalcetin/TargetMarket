...
public class TM_SC_02 extends Hooks {
	...
	void testSkinBeautySerumImage() {
		try {
			...
			ExtentReportListener.getTest()
				.info("4-) Verify that its image is displayed properly.\n"
						+ "\t\t// softAssert.assertTrue(pages.getTargetMarketHomePage().isProductPictureLoadedProperly(\"Skin\n"
						+ "\t\t// Beauty Serum.\"),\n" + "\t\t// \"Product picture is not loaded properly\");");
	...
	void checkSkinBeautySerumAddCartFunction() {
		try {
			...
			// 9-) Verify that button in Skin Beauty Serum. card is turned to "Added to
			// Cart".
			ExtentReportListener.getTest()
				.info(" 9-) Verify that button in Skin Beauty Serum. card is turned to \"Added to Cart\".");
			softAssert.assertEquals(pages.getTargetMarketHomePage().getButtonText("Skin Beauty Serum."),
					"Added to Cart", "Text button doesn't turn to 'Added to Cart'");
