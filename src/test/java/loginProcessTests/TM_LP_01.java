	...
	public void testLoginProcessWithDifferentUsers(String username, String password) {
		...
		try {
			ExtentReportListener.getTest()
				.info("1- Create a DataProvider method to provide different user credentials for the test");
			ExtentReportListener.getTest()
				.info("2- Create Test method using the DataProvider to run multiple test cases with\n"
						+ "        // different users");
			...
			// clickLogin())
			ExtentReportListener.getTest()
				.info("4- Login using the provided username and password(Login method calls\n"
						+ "        // clickLogin())");
			pages.getTargetMarketLoginPage().login(username, password);

			// 5- Verify the welcome text on the Target Market home page after login
			ExtentReportListener.getTest()
				.info("5- Verify the welcome text on the Target Market home page after login");
